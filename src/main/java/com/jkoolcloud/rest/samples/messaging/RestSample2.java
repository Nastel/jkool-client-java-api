	...
	public static void main(String[] args) {
		try {
			...
			Event event = new Event().setSourceUrl("https://www.sample.com/orders/parts").setSeverity(Severities.INFO)
			        .setType(EventTypes.RECEIVE).setPid(7373).setTid(Thread.currentThread().getId())
			        .setCompCode(CompCodes.SUCCESS).setReasonCode(0).setLocation("Los Angeles, CA")
			        .setUser("ebay-proc").setElapsedTimeUsec(45938).setMsgText("OrderId=28372373 payment processed.")
			        .setMsgEncoding("none").setMsgCharset("windows-1252").setCorrId(Arrays.asList("CorrId:123"))
			        .setResource("PAYMENT.QUEUE").setMsgMimeType("text/plain").setMsgAgeUsec(0).setWaitTimeUsec(0)
			        .setEventName("ReceivePayment").setAppl("WebPayments")
			...
			// (RestSample2 class)
			event = new Event().setSourceUrl("https://www.sample.com/orders/parts").setSeverity(Severities.INFO)
			        .setType(EventTypes.SEND).setPid(7373).setTid(Thread.currentThread().getId())
			        .setCompCode(CompCodes.SUCCESS).setReasonCode(0).setLocation("Los Angeles, CA").setUser("webuser")
			        .setElapsedTimeUsec(107).setMsgText("Verify ProductId=28372373").setMsgEncoding("none")
			        .setMsgCharset("windows-1252").setCorrId(Arrays.asList("CorrId:123")).setResource("SHIPPING.QUEUE")
			        .setMsgMimeType("text/plain").setMsgAgeUsec(0).setException(null).setMsgTag(null)
			        .setParentTrackId(null).setWaitTimeUsec(0).setEventName("ProcessPayment").setSnapshots(null)
